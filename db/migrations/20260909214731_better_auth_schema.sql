-- migrate:up
-- Generated by `npx auth generate` against better-auth 1.7.3 with the
-- organization plugin (teams enabled), then committed verbatim. The
-- committed copy in db/better-auth-schema.sql is the drift-check baseline;
-- see `pnpm run auth:schema:check`. Do not edit this file after it has
-- been applied - schema changes arrive as new migrations.
--
-- Identifier quoting is load-bearing: "user" is a reserved word in
-- Postgres and "teamMember" is camelCase, so both fold or fail without
-- quotes. Every column here is camelCase and must be quoted in raw SQL.
